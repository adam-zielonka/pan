...
const CardElement = ({ card, disabled }: { card: Card; disabled: boolean }) => {
...
const PlayerID = observer(({ id }: { id: number }) => {
  ...
  return (
    <div
      className={`bp3-dialog-header playerID${id + 1}`}
      style={{ backgroundColor: token === id ? 'lightgreen' : 'white' }}
    >
...
const PlayerElement = ({
  ...
  return (
    <div className="cards bp3-card player1">
      ...
      {player.getFigureActions(isStackEmpty).map(figure => (
        <button
          key={figure}
          className="card"
...
const OpponentElement = ({ player }: { player: IPlayer }) => {
  return (
    <div className={'cards bp3-card player' + (player.id + 1)}>
      <button disabled={true} className="card">
        {player.cards.length}
      </button>
    </div>
  )
  ...
  return (
    <div className="cards bp3-card stack">
      <div className="bp3-dialog-header">
        <span
          className="bp3-icon-large bp3-icon-layers"
          style={{ color: 'black' }}
        ></span>
      </div>
      {stack.length > 3 && (
      ...
      {stack.slice(Math.max(stack.length - 3, 0)).map(card => (
        <CardElement key={card.toString()} card={card} disabled={true} />
      ))}
      <button
        className="card"
      ...
      {players
        ...
        .map((player, i) => (
          <>
            ...
            <OpponentElement key={i} player={player} />
          </>
        ))}
      <Stack />
      <PlayerID id={0} />
      <PlayerElement player={players[0]} isStackEmpty={stack.length ? true : false} />
    </div>
  )
}
